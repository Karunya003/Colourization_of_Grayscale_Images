...
import cv2 as cv
import os.path
numpy_file = np.load(r'pts_in_hull.npy')
Caffe_net = cv.dnn.readNetFromCaffe(r"models\colorization_deploy_v2.prototxt", r"models\colorization_release_v2.caffemodel")
...
class Window(Frame):
    ...
    def uploadImage(self):
        ...
        cv.imwrite(r"result.png", (bgr_output*255).astype(np.uint8))
    def color(self):
        load = Image.open(r"result.png")
        load = load.resize((480, 360),Image.LANCZOS)
        if self.image is None:
            w, h = load.size
            self.render = ImageTk.PhotoImage(load)
    ...
    def saveImage(self):
    # Get the current output image displayed on the canvas
        if self.image3:
            image = self.render3
            if image:
        # Prompt the user to choose a save location
            ...
            # Save the image using PIL
                    output_image = ImageTk.getimage(image)
                    output_image.save(filename)
 
root = tk.Tk()
root.geometry("%dx%d" % (980, 600))
root.title("B&W Image Colorization GUI")
img = ImageTk.PhotoImage(Image.open(r"new1.jpg"))
app = Window(root)
